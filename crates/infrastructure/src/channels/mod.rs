...
//! module per provider (mirrors `postgres`, one module per aggregate). Every
//! adapter here is a mock/sandbox implementation (prompt 08: "first adapters
//! may be mock/sandbox implementations if external credentials are not
//! available") — no vendor SDK or real network call yet, but endpoint
//! validation and provider-error encapsulation are real and independently
//! testable, so swapping in a real HTTP client later only touches `send`.
